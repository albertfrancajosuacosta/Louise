...
class JanelaHipoteseNaoParametricoMais2Grupos(Toplevel):
    ...
    def criarJanela(self):
        ...
        listaTestes  = ['Friedman','Kruskal-Wallis']
    ...
    def processarArquivo(self,caminho):
        ...
        try:
            ...
            if self.qtdColunasPlanilha <= 2:
                Messagebox.show_error(title="Erro", message="Os dados precisam estar em três ou mais colunas. Atualmente os dados estão em "+str(self.qtdColunasPlanilha)+' colunas.')

            else:

              if self.qtdColunasPlanilha > 6:
                  Messagebox.show_error(title="Erro",
                                        message="A presente versão de Louise está limitada à 6 grupos. Atualmente há " + str(
                                            self.qtdColunasPlanilha) + ' grupos.')
              else:
    ...
    def carregarDados(self,planilha):
        ...
        self.st.insert(END,self.util.formatarCabecalho2MaisGrupos(self.qtdColunasPlanilha))
        for index, row in planilha.iterrows():

            _texto = self.util.formatarLinha2MaisGrupos(self.qtdColunasPlanilha,index,row)
    ...
    def testeFriedmans(self):
        self.textoResultado.limpar()
        te = TesteEstatistico(signi=self.sig)
        estatistica, p_value = te.friedman(self.planilha,self.qtdColunasPlanilha)
